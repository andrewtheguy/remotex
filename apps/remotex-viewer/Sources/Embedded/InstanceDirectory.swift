...
struct InstanceDirectory: Equatable, Sendable {
    ...
    /// Chromium's profile for this instance — `cache_path`, and the cookie jar
    /// and `localStorage` under it.
    ///
    ...
    /// profile has to persist, or every launch forgets them, silently, with nothing
    /// on screen to say a preference was dropped.
    ///
    /// Simply a path inside this directory, which is what makes `--instance-dir`
    /// isolate the preferences the same way it isolates the config and the log. The
    /// WebKit arrangement this replaces could not be a path: `WKWebsiteDataStore`
    /// keeps its store in the app's own container whatever you ask, so the
    /// directory had to be hashed down to a `UUID` and handed over as an identifier
    /// — a whole mechanism to buy what naming a subdirectory buys here.
    var browserProfile: URL {
        // Standardized so `/tmp/x` and `/tmp/./x` are one instance, which is what
        // they are to everything else here.
        url.standardizedFileURL.appending(path: "chromium", directoryHint: .isDirectory)
    }
