...
///
/// Input only, plus what the app cannot see for itself: whether the page came
/// up able to decode audio, whether sound is actually playing, and whether a
/// video target's stream is decoding.
enum CanvasEvent: Equatable, Decodable {
    ...
    /// Why a target on `render_type = "video"` is not painting, or nil once it
    /// is. Not folded into `ready` the way the audio decoder is: only a video
    /// target can reach this, most targets are not one, and a decoder nothing
    /// will ask for is nothing to report.
    case videoState(error: String?)
    ...
    init(from decoder: any Decoder) throws {
        ...
        case "videoState":
            self = .videoState(error: try container.decodeIfPresent(String.self, forKey: .error))
        default:
