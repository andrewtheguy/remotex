...
struct CanvasCommandTests {
    ...
    /// The video report, both ways round. A target on `render_type = "video"`
    /// sends no still tiles, so a stream that will not decode is a window that
    /// stays black — the error is the only thing that says why, and the null is
    /// the stream having started.
    @Test
    func aVideoStateCarriesItsReasonOrTheAbsenceOfOne() throws {
        #expect(
            event(#"{"type":"videoState","error":"no decoder"}"#)
                == .videoState(error: "no decoder")
        )
        #expect(
            event(#"{"type":"videoState","error":null}"#) == .videoState(error: nil)
        )
    }
