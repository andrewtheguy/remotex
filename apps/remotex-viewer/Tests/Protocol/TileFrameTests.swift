...
struct TileFrameTests {
    ...
    func aWellFormedRecordDecodes() throws {
        ...
        let tiles = try #require(
            BatchFrame.decode(
                batch(tile(format: 2, slot: 5, x: 16, y: 320, w: 1280, h: 64, payload: payload))
            )
        )
    ...
    func anEmptyBatchDecodesToNoRecords() throws {
        let tiles = try #require(BatchFrame.decode(batch()))
    ...
    func aRecordWithNoPayloadDecodesToAnEmptyPayload() throws {
        let tiles = try #require(BatchFrame.decode(batch(tile(format: 1, w: 8, h: 8))))
        #expect(tiles[0].payload.isEmpty)
    }
    ...
    func aSliceWithANonZeroStartIndexDecodesTheSameWay() throws {
        ...
        let tiles = try #require(BatchFrame.decode(slice))
        #expect(tiles[0].x == 7)
        #expect(tiles[0].y == 9)
        #expect(Array(tiles[0].payload) == payload)
    }
