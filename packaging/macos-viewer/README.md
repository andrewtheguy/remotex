...
a random token delivered through a private pipe, shows the SPA out of its own
bundle in an embedded **Chromium**, and stops the gateway when the app quits.

The page is served to itself as `remotex://app` rather than fetched from the
gateway so that its origin — and with it the client's remembered preferences —
survives a launch on a different port. The gateway serves that same directory, so a
browser pointed at the embedded port gets the same client.
