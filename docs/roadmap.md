...
- **Phase 7 — auth support:** a web login guarding the whole surface (SPA,
  `/api/*`, `/ws`), remotex-style: a `site_passwd` credential in the TOML
  (base64 `username:bcrypt_hash`, generated by a helper tool), a login
  endpoint that sets a session cookie, and everything else refusing
  unauthenticated requests. Target credentials already never leave the
  server; this closes the gap where anyone who can reach the port can use
  the desktop.
