...
/// The logged-in user's name and avatar. Advertised and stepped over.
...
/// nothing to pick — while this exact sequence produced one every time. Sixteen
/// variants, one connection each, no exceptions. Why the daemon reads the list this
/// way is unresolved; that it does is not. See docs/apple-vnc-889.md.
///
...
pub fn parse_layout(payload: &[u8]) -> anyhow::Result<Layout> {
    ...
    for (index, record) in payload[LAYOUT_HEAD..].chunks(LAYOUT_RECORD).enumerate() {
        ...
        anyhow::ensure!(
            stated.is_finite() && (1.0..=4.0).contains(&stated),
            "a display layout states a scale factor of {stated}, outside 1..=4"
        );
...
mod tests {
    ...
    /// Build a layout payload at the *measured* offsets, for the cases a capture
    /// cannot cover — a mirrored screen, a malformed length. Cross-checked against
    /// [`TWO_REAL_SCREENS`] by [`the_builder_agrees_with_the_captured_bytes`], so a
    /// drift between the two shows up as a failure rather than as agreement with
    /// itself.
    fn layout(current: Option<u32>, displays: &[Screen]) -> Vec<u8> {
        ...
        // framebuffer, which is the first screen's when one is selected.
        let span: u16 = displays.iter().map(|d| d.1.0).sum();
        // Selected: that screen's own pixels. Combined: the union of every screen's,
        // which is what makes a mixed-density Mac's framebuffer taller and wider than
        // any single scale explains.
        ...
        // The Mac stops two bytes short of the last record, and `declared` counts
        // them anyway. A builder that did not do this would let the parser's length
        // rule drift without a single test noticing.
    ...
    fn a_layout_that_does_not_add_up_is_refused() {
        ...
        let mut payload = one(0x01);
        payload[LAYOUT_HEAD + 0x02..LAYOUT_HEAD + 0x0a].copy_from_slice(&[0, 0, 0x3f, 0xf0, 0, 0, 0, 0]);
        let err = parse_layout(&payload).unwrap_err();
        assert!(format!("{err:#}").contains("scale factor"), "{err:#}");
    }
