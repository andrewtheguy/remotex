...
impl SessionManager {
    ...
    /// queue into the engine — real work that must not sit on it.
...
mod tests {
    ...
    async fn a_camera_socket_drives_the_engines_bridge_both_ways() {
        let (mgr, hooks) = manager_with_fake_engine();
        let (token, _att, bridge, recorder, _ends) = connected_camera_session(&mgr, &hooks).await;
        ...
        bridge.signal(CameraSignal::Start(CAM_FORMAT));
        ...
        assert_eq!(signal, CameraSignal::Start(CAM_FORMAT));
    }
    ...
    async fn closing_the_camera_socket_unplugs_the_device() {
        let (mgr, hooks) = manager_with_fake_engine();
        let (token, _att, _bridge, recorder, _ends) = connected_camera_session(&mgr, &hooks).await;
    ...
    async fn a_disconnect_closes_the_camera_socket_with_the_engine() {
        let (mgr, hooks) = manager_with_fake_engine();
        let (token, att, _bridge, _recorder, _ends) = connected_camera_session(&mgr, &hooks).await;
