...
//! hash verbatim — its `$./` alphabet needs no escaping): `[server].site_passwd`
//! is plain `username:bcrypt_hash` (generated by `rdpweb gen-passwd <username>`),
//! `/api/auth/login` verifies a username/password pair against it and mints a
//! session token, and everything guarded (the rest of `/api/*`, `/ws`) requires
//! that token in the cookie. Sessions live in memory only — a server restart
//! logs every browser out, which is fine for a single-user program.
