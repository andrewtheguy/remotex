//! Who may talk to this gateway: the web login, and nothing else.
//!
//! TOML strings carry a bcrypt hash verbatim — its `$./` alphabet needs no
//! escaping: `[server].site_passwd` is plain `username:bcrypt_hash` (generated by
//! `remotex gen-passwd <username>`), `/api/auth/login` verifies a username/password
//! pair against it and mints a session token, and everything guarded (the rest of
//! `/api/*`, `/ws`) requires that token in the cookie. Sessions live in memory only
//! — a server restart logs every browser out, which is fine for a single-user
//! program.
//!
//! The cookie is the carrier rather than a header because it has to reach two
//! kinds of request from inside one document: `fetch` to `/api/*` and the `/ws`
//! upgrades, neither of which a page can attach a header to.
