...
- headless Playwright is allowed only for DOM/control-plane flows that have proved stable; the current whitelist is `tests/playwright/clipboard.spec.js` (login, target selection, menu/panel state, clipboard metadata/reveal/copy/send, and responsive panel docking) and `tests/playwright/oversized-clipboard.spec.js` (a remote clipboard over `MAX_CLIPBOARD_BYTES` reported as its size rather than truncated). Shared login/target and SSH pasteboard helpers live in `tests/playwright/support.js`. Preserve approved tests in `tests/playwright/` instead of leaving one-off copies in `tmp/`; add another flow to this whitelist only after repeated local passes
