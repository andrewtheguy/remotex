...
- **never open the app from a mounted DMG, and eject the image when the check above is done.** `SMAppService` registers the bundle's embedded plist, whose `BundleProgram` is a path *relative to the registering bundle*, and every copy registers itself on launch — so a copy opened from `/Volumes/…` captures the launchd job permanently. `launchctl kickstart -k` then starts that old binary silently, or nothing at all once the image is ejected, and it survives reboots
- **after redeploying the agent, verify what is actually running rather than what is installed.** `--version` on the bundle and the version in the log prove nothing about the job launchd holds. `lsof -p $(pgrep -x remotex-agent) | awk '$4=="txt"'` names the executing bundle; `launchctl print gui/<uid>/dev.remotex.agent | grep 'parent bundle version'` names the registered one. If they disagree, `kickstart` cannot fix it — kill the stray process, eject any DMG, and `open -a /Applications/remotex-agent.app` so the installed copy re-registers and repoints the job. Removing this whole failure mode is planned (`docs/roadmap.md`)
