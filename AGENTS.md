...
not play": the wrapper sets FreeRDP's `RemoteConsoleAudio` whenever no sink is
given, so the host keeps its own speakers and a headset paired to it stays usable
for the session. That is what off already means on every VNC target, which can
take nothing away from the remote. Windows reads the mode once, from the Client
Info PDU, so it cannot change mid-session; a target is one mode or the other.
...
Don't test camera on windows server because it requires special trial feature to be installed to work. For normal qa flow windows camera redirection is not needed.
