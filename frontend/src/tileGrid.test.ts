...
test("the lattice is the gateway's cells, interior lines only", () => {
  ...
  assert.deepEqual(tileGridLines({ w: 192, h: 128 }, { w: 64, h: 64 }), {
    xs: [64, 128],
    ys: [64],
  });
});
...
test("a desktop that does not divide evenly keeps every whole boundary", () => {
  // 1920x1080 is 30 whole columns and 16 whole rows plus a 56px remainder: the
  // partial cell at the bottom has a boundary above it and none below.
  const { xs, ys } = tileGridLines({ w: 1920, h: 1080 }, { w: 64, h: 64 });
  assert.equal(xs.length, 29);
  assert.deepEqual(xs.slice(0, 3), [64, 128, 192]);
...
test("a desktop smaller than one cell has no interior boundary to draw", () => {
  assert.deepEqual(tileGridLines({ w: 50, h: 40 }, { w: 64, h: 64 }), {
    xs: [],
    ys: [],
  });
});
...
test("a nonsense pitch draws nothing rather than looping on it", () => {
  ...
  for (const bad of [0, -64, 0.5, Number.NaN, Number.POSITIVE_INFINITY]) {
    assert.deepEqual(
