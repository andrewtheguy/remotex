...
// tests/playwright/mac-keyboard.spec.ts and manual QA. Run with
...
// macOS browsers can withhold `keyup` for a character key while Command is held.
// Without the flush, the guest is left holding both the letter and a Control it
// can never be told to release.
...
// A browser is never given ⌘W — the tab closes first — so it is not in the table
// and Command is forwarded as itself, which is what leaves the guest able to see
// the chord it was actually sent. docs/roadmap.md's companion extension is what
// would change that.
test("a browser forwards Command-W rather than mapping it", () => {
